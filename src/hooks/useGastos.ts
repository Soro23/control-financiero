...
export function useGastos(year: number, month: number) {
  ...
  async function createGasto(data: ExpenseFormData): Promise<boolean> {
    ...
    try {
      await addDoc(collection(db, "users", userId, "expense_entries"), {
        ...data,
        subcategory_id: data.subcategory_id || null,
        notes: data.notes || null,
  ...
  async function updateGasto(id: string, data: Partial<ExpenseFormData>): Promise<boolean> {
    if (!userId) return false;
    try {
      await updateDoc(doc(db, "users", userId, "expense_entries", id), {
        ...data,
        subcategory_id: data.subcategory_id || null,
        notes: data.notes || null,
        updated_at: new Date().toISOString(),
  ...
  async function deleteGasto(id: string): Promise<boolean> {
    if (!userId) return false;
    try {
      await deleteDoc(doc(db, "users", userId, "expense_entries", id));
      await fetchEntries();
      return true;
    } catch { return false; }
  }
